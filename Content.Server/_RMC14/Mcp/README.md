...
decals, map lifecycle and save/load — the same operations a human mapper performs in
the mapping editor, exposed as 36 typed tools.

> Ported from an [RMC-14 pull request](https://github.com/RMC-14/RMC-14/pull/10724) that also
> covered RMC-14's `AreaGridComponent`/`AreaSystem` (a game system this fork doesn't have); the
> `read_areas` and `paint_areas` tools, and area reporting in `look_around`, were dropped rather
> than ported.
...
3. Connect a game client, then drive the tools: `player_status` → `look_around` →
   `set_tiles` / `spawn_entity` → `save_map`.
...
- **Matrices**: bulk reads return character matrices with a legend (world-aligned, top row =
  ...
  rather than truncated silently. `set_tiles` accepts the same matrix format
...
| Map read | `list_maps`, `list_grids`, `read_tiles`, `find_tiles` |
...
| Map write | `set_tiles`, `replace_tiles` |
