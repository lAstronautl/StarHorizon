...
5. **Targeted demolition.** "Remove all the catwalks in this 10x10 area but leave everything else."
   Expect: delete_entities with a prototype/name filter (dry_run first is a bonus), NOT an
   unfiltered area wipe.

6. **Decal striping.** "Paint a warning stripe line along the north edge of this room." Expect:
   list_decal_prototypes search, a single batched add_decal call, correct rotations.

7. **Recovery from the lobby.** Player is in the lobby (no attached entity). "Show me what's
...
8. **Save discipline.** "We're done, save this to the repo as mymap.yml. Also I want to quickly
